...
// Get foosball info
$foosball = $db->select("SELECT wins, elo, @rownum:=@rownum + 1 AS rank FROM FoosballSingles ORDER BY elo ASC LIMIT 25;");
if($foosball != false){
...
// Get pingpong info
$pingpong = $db->select("SELECT wins, elo, @rownum:=@rownum + 1 AS rank  FROM PingPongSingles ORDER BY elo ASC LIMIT 25;");
if($pingpong != false){
